let () =
  let allowed_ip : int -> Wglib.Wgapi.Allowed_ip.t =
    ...
    let ip =
      Wglib.Wgapi.Allowed_ip.Ip.V4
        (Wglib.Wgapi.Allowed_ip.Ip.V4.of_string_exn ("0.0.0." ^ Int.to_string i))
    in
  ...
  let endpoint : Wglib.Wgapi.Endpoint.t =
    { addr = `V6 (Ipaddr.V6.of_string_exn "2001:0000:130F:0000:0000:09C0:876A:130B")
    ; port = 4321
    }
  in
  let _peers =
  ...
  let () = Wglib.Wgapi.Interface.remove_peers device device.peers |> Result.get_ok in
